...
bool sendMessage(amqp_connection_state_t conn, string exchange, string routingkey, string message,string queue) {
	amqp_basic_properties_t props;
	props._flags = AMQP_BASIC_CONTENT_TYPE_FLAG | AMQP_BASIC_DELIVERY_MODE_FLAG;
	props.content_type = amqp_cstring_bytes("text/plain");
	props.delivery_mode = 2; /* persistent delivery mode */
	amqp_basic_consume_ok_t* r =
		amqp_basic_consume(conn, 1, amqp_cstring_bytes(queue.c_str()),amqp_cstring_bytes(exchange.c_str()), 0, 0, 0,amqp_empty_table);
      ...
      try
        {
          ...
          if (sendMessage(conn, exchange, routingkey, strData,queue)) {
              cout << "send........" << endl;
          }
          else {
